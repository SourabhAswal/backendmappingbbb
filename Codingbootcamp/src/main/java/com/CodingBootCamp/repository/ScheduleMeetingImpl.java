...
public class ScheduleMeetingImpl implements ScheduleMeetingService {
			...
			if (datecheck != 0) {
				for (ScheduleMeeting ms : meetings) {
					if (m.getDate().compareTo(ms.getDate()) == 0) {
						...
						System.out.println("coming after slot- " + afterSlot + "coming before slot " + beforeSlot);
	...
	private CreateMeetingPayload createMeeting(ScheduleMeeting m) {
		...
		String createMeeting = "name="+m.getMeeting_name() + random_int + "&meetingID=bootcamp" + random_int + "&moderatorPW=moderator" + "&attendeePW=attendee";

		HttpHeaders headers = new HttpHeaders();
		CreateMeetingPayload response = restTemplate.getForObject(bigBlueButtonURL + "create?" + createMeeting + "&checksum=" + DigestUtils.shaHex("create" + createMeeting + secret), CreateMeetingPayload.class);

		//printing the created url
		System.out.println(bigBlueButtonURL + "create?" + createMeeting + "&checksum=" + DigestUtils.shaHex("create" + createMeeting + secret));
	...
	private CreateMeetingPayload joinMeetingAsModerator(ScheduleMeeting m) {
		...
		String joinMeeting = "fullName=Moderator" + random_int + "&meetingID=" + m.getMeeting_id() + "&password=" + m.getModeratorPswd() + "&redirect=true";

		HttpHeaders headers = new HttpHeaders();

		//printing join as moderator url
		String moderatorUrl=bigBlueButtonURL + "join?" + joinMeeting  + "&checksum=" + DigestUtils.shaHex("join"+ joinMeeting + secret);
		m.setModerator_link(moderatorUrl);

		System.out.println(" moderator url :  "+ bigBlueButtonURL + "join?" + joinMeeting  + "&checksum=" + DigestUtils.shaHex("join"+ joinMeeting + secret) );
		
		//calling joinMeetingAsAttendee()
		joinMeetingAsAttendee(m);
		
		CreateMeetingPayload response = restTemplate.getForObject(bigBlueButtonURL + "join?" + joinMeeting  + "&checksum=" + DigestUtils.shaHex( joinMeeting + secret), CreateMeetingPayload.class);
		return response;
	}
		...
		private CreateMeetingPayload joinMeetingAsAttendee(ScheduleMeeting m) {
			...
			String joinMeeting = "fullName=Attendee" + random_int + "&meetingID=" + m.getMeeting_id() + "&password=" + m.getAttendeePswd() + "&redirect=true";

			HttpHeaders headers = new HttpHeaders();

			//printing join as attendee url
			String attendeeUrl=bigBlueButtonURL + "join?" + joinMeeting  + "&checksum=" + DigestUtils.shaHex("join"+ joinMeeting + secret);
			m.setMeeting_link(attendeeUrl);

			System.out.println(" attendee url :  "+ bigBlueButtonURL + "join?" + joinMeeting  + "&checksum=" + DigestUtils.shaHex("join"+ joinMeeting + secret) );
			CreateMeetingPayload response = restTemplate.getForObject(bigBlueButtonURL + "join?" + joinMeeting  + "&checksum=" + DigestUtils.shaHex( joinMeeting + secret), CreateMeetingPayload.class);
			return response;
		}
